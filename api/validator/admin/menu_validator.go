...
// CreateMenuValidator 添加菜单验证
func CreateMenuValidator(param dto.CreateMenuRequest) error {
	if param.MenuName == "" {
		return errors.New("请输入菜单名称")
	}
	if utils.Contains([]string{menu_key.MENU_TYPE_DIRECTORY, menu_key.MENU_TYPE_MENU}, param.Path) && param.Path == "" {
		return errors.New("请输入路由地址")
	}
	if param.IsFrame == menu_key.MENU_YES_FRAME && !strings.HasPrefix(param.Path, "http") {
		return errors.New("新增菜单" + param.MenuName + "失败，地址必须以http(s)://开头")
	}
	return nil
}

// UpdateMenuValidator 更新菜单验证
